...
test(`${device.name}: delete hidden file without unlock returns 200 but leaves file on device`, async () => {
  await api.devices.unlockProtectedFiles(device.serialNumber);
  ...
  try {
    const { ok } = await api.devices.deleteDeviceFile(device.serialNumber, TEST_HIDDEN_FILENAME);
    expect(ok).toBe(true);

    const { data: stillThere } = await api.devices.getDeviceFileList(device.serialNumber);
    expect(stillThere).toContain(TEST_HIDDEN_FILENAME);
  } finally {
    await api.devices.unlockProtectedFiles(device.serialNumber);
